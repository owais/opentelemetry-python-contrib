...
def main():
    ...
    for pkg in os.listdir(base_instrumentation_path):
        ...
        if not meta["instruments"]:
            default_instrumentations.elts.append(ast.Str(instrumentation_package))
        for target_pkg in meta["instruments"]:
            ...
            libraries.values.append(
                ast.Dict(
                    keys=[ast.Str("versions"), ast.Str("instrumentation")],
                    values=[
                        ast.Str("".join(parsed.specs[0]) if parsed.specs else ""),
